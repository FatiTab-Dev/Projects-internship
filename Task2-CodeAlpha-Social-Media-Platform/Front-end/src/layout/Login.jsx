...
export const Login = ({ onLogin }) => {
  ...
  return (
    <div className="login container d-flex justify-content-center align-items-center" style={{ minHeight: '100vh', backgroundColor: '#f4f7f6' }}>
      <div className="card p-4 shadow-lg border-0 rounded-4" style={{ width: '100%', maxWidth: '400px' }}>
        
        {/* tab */}
        <div className="d-flex justify-content-center mb-4">
          <button className={`btn fw-bold px-4 ${activeTab === 'login' ? 'text-primary border-bottom border-primary' : 'text-muted'}`} onClick={() => setActiveTab('login')}>Login</button>
          <button className={`btn fw-bold px-4 ${activeTab === 'signup' ? 'text-primary border-bottom border-primary' : 'text-muted'}`} onClick={() => setActiveTab('signup')}>Sign up</button>
        </div>
