...
const updateProject = async () => {
    try {
        ...
        const data = await res.json();
        setProject(prev => prev.map(p => p._id === editingProject._id ? data : p));
 ...
 return (
    ...
    <h5 onClick={() => navigate(`/project/${p._id}`)} style={{cursor:'pointer'}}>{p.title}</h5>
    <p>{p.description}</p>
    <div className='d-flex gap-2 mt-2'>
        <button className='btn btn-sm btn-outline-warning' onClick={(e) => { e.stopPropagation(); setEditingProject(p); }}>Edit</button>
        <button className='btn btn-sm btn-outline-danger' onClick={(e) => { e.stopPropagation(); deleteProject(p._id); }}>Delete</button>
        <button className='btn btn-sm btn-outline-info' onClick={(e) => { e.stopPropagation(); setInvitingProject(p._id); }}>Invite</button>
    </div>
</div>
        </div>
    ))}
        ...
        <div className="modal-dialog">
            <div className="modal-content">
                <div className="modal-header">
                    ...
                    <button className="btn-close" onClick={() => setInvitingProject(null)}></button>
                </div>
                <div className="modal-body">
                    <input className="form-control" placeholder="Email" value={inviteEmail} onChange={(e) => setInviteEmail(e.target.value)} />
                </div>
                <div className="modal-footer">
                    <button className="btn btn-secondary" onClick={() => setInvitingProject(null)}>Cancel</button>
                    <button className="btn btn-info" onClick={inviteMember}>Invite</button>
                </div>
            </div>
        </div>
    </div>
